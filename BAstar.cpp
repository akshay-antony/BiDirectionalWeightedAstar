...
bool is_not_obstacle(float x, float y, std::map<std::pair<float,float>,std::vector<float>>& all_points){
...
bool is_in_closed_set(float x, float y, std::set<std::pair<float,float>>& closed_points){
...
float find_h(float x, float y, bool forward){
    if(forward)
